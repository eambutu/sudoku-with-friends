...
Cell.prototype.element = function () {
  ...
  if (this.selected) cell.classList.add("selected");
  if (this.digit != 0) cell.classList.add("filled");
  else cell.classList.add("empty");
  if (this.prefilled) cell.classList.add("prefilled");
  ...
  for (let row = 0; row < 3; row++) {
    for (let col = 0; col < 3; col++) {
      let svg_candidate_button = document.createElementNS(
  ...
  cell.id = this.id();
  let svg_number = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "svg"
  );
      ...
      for (let interaction of INTERACTIONS) {
        if (interaction.has(JSON.stringify({ x: cell.x, y: cell.y }))) {
          for (let pos_as_str of interaction) {
            ...
            if (
              cell.digit != 0 &&
              cell.digit == this.cells[pos.x][pos.y] &&
  ...
  for (let cell of this.dirty_cells) {
    if (document.getElementById(cell.id())) {
      document
        ...
        .replaceChild(cell.element(), document.getElementById(cell.id()));
    } else {
...
Grid.prototype.select_cell = function (cell) {
  if (this.selected_cell != cell) {
    document
      .getElementById(this.selected_cell.id())
...
Grid.prototype.update_cell = function (data) {
  ...
  if (typeof data.digit != "undefined") {
    if (cell.digit != data.digit) {
      ...
      if (this.digits_left[data.digit] < 1 && data.digit != 0) {
        document
          .getElementById(`keyboard-${data.digit}`)
          .classList.add("no_digits_left");
      } else if (data.digit != 0) {
        document
          .getElementById(`keyboard-${data.digit}`)
          .classList.remove("no_digits_left");
      }
      if (this.digits_left[cell.digit] < 1 && cell.digit != 0) {
        document
          .getElementById(`keyboard-${cell.digit}`)
          .classList.add("no_digits_left");
      ...
      for (let interaction of INTERACTIONS) {
        if (interaction.has(JSON.stringify({ x: cell.x, y: cell.y }))) {
          for (let pos_as_str of interaction) {
            ...
            if (
              cell.digit != 0 &&
              cell.digit == this.cells[pos.x][pos.y].digit &&
              !(cell.x == pos.x && cell.y == pos.y)
            ) {
              cell.add_conflict(this.cells[pos.x][pos.y]);
            }
  ...
  if (typeof data.modify_candidate != "undefined") {
    if (data.modify_candidate.remove) {
      ...
      document
        .getElementById(cell.svg_candidate_id(data.modify_candidate.candidate))
...
Grid.prototype.show_selected = function (data) {
  ...
  if (currently_selected) {
    ...
    if (currently_selected.other_players_selected.length == 0) {
      document
        .getElementById(currently_selected.id())
        .classList.remove("other-selected");
    }
  }
  ...
  document
    .getElementById(this.other_selected.get(data.color).id())
    .classList.add("other-selected");
  document.getElementById(
    currently_selected.id() + "-other-selected"
...
socket.on("set up board", (board) => {
  ...
  document.addEventListener("keydown", function (event) {
    ...
    if (event.code == "Digit1") f.call(grid, 1);
    else if (event.code == "Digit2") f.call(grid, 2);
    else if (event.code == "Digit3") f.call(grid, 3);
    else if (event.code == "Digit4") f.call(grid, 4);
    else if (event.code == "Digit5") f.call(grid, 5);
    else if (event.code == "Digit6") f.call(grid, 6);
    else if (event.code == "Digit7") f.call(grid, 7);
    else if (event.code == "Digit8") f.call(grid, 8);
    else if (event.code == "Digit9") f.call(grid, 9);
    else if (event.code == "Backspace" || event.code == "Delete")
      grid.set_digit(0);
    else if (event.code == "ArrowLeft") {
      grid.select_cell(
        grid.cells[grid.selected_cell.x][
          (((grid.selected_cell.y - 1) % 9) + 9) % 9
        ]
      );
      event.preventDefault();
    } else if (event.code == "ArrowRight") {
      grid.select_cell(
        grid.cells[grid.selected_cell.x][(grid.selected_cell.y + 1) % 9]
      );
      event.preventDefault();
    } else if (event.code == "ArrowDown") {
      grid.select_cell(
        grid.cells[(grid.selected_cell.x + 1) % 9][grid.selected_cell.y]
      );
      event.preventDefault();
    } else if (event.code == "ArrowUp") {
      grid.select_cell(
        grid.cells[(((grid.selected_cell.x - 1) % 9) + 9) % 9][
          grid.selected_cell.y
        ]
      );
      event.preventDefault();
    }
  ...
  for (let pad_digit = 1; pad_digit < 10; pad_digit++) {
    document
      .getElementById(`keyboard-${pad_digit}`)
      .addEventListener("click", function () {
        if (MODE == NORMAL_MODE) {
  ...
  document
    .getElementById(`keyboard-delete`)
    .addEventListener("click", function () {
      grid.set_digit(0);
    });

  document
    .getElementById(`keyboard-normal`)
    .addEventListener("click", function () {
      MODE = NORMAL_MODE;
      document.getElementById("keyboard-normal").classList.add("normalMode");
      document
        .getElementById("keyboard-candidate")
        .classList.remove("candidateMode");
